...
class IkshanaNet(nn.Module):
  def __init__(self):
    ...
    self.s1 = nn.Sequential(nn.Conv2d(96, 12, kernel_size=1, stride =1,bias=False),
                                   nn.BatchNorm2d(12),
                                   nn.ReLU(inplace=True))
    self.pool = nn.AvgPool2d(kernel_size=2)
    self.glance21 = look(in_filters=99, dilation_rate=1)
    self.glance22 = look(in_filters=131, dilation_rate=2)
    self.glance23 = look(in_filters=163, dilation_rate=3)
    
    
    self.s2 = nn.Sequential(nn.Conv2d(192, 12, kernel_size=1, stride =1,bias=False),
                                   nn.BatchNorm2d(12),
                                  nn.ReLU(inplace=True))
    
    self.glance31 = look(in_filters=195, dilation_rate=1)
    self.glance32 = look(in_filters=227, dilation_rate=2)
    self.glance33 = look(in_filters=259, dilation_rate=3)



    self.s3 = nn.Sequential(nn.Conv2d(288, 12, kernel_size=1, stride =1,bias=False),
                                  nn.BatchNorm2d(12),
                                  nn.ReLU(inplace=True))
...
def train_val(model, params):
    ...
    best_loss=float('inf')
    o = open('./Ikshana3G-2-Camvid-BS2/Ikshana3G-2.txt','w')
...
import os 
path2models= "./Ikshana3G-2-Camvid-BS2/Ikshana3G-2_"
...
o = open('./Ikshana3G-2-Camvid-BS2/Ikshana3G-2-time.txt','w')
...
plt.legend()
plt.savefig('./Ikshana3G-2-Camvid-BS2/Ikshana3G-2.png', dpi = 300)
...
print(iou_sum/101)
o = open('./Ikshana3G-2-Camvid-BS2/Ikshana3G-2_IOU_val.txt','w')
...
print(i_sum/233)
o = open('./Ikshana3G-2-Camvid-BS2/Ikshana3G-2_IOU_test.txt','w')
...
with open('./Ikshana3G-2-Camvid-BS2/plot.csv', 'w') as f:
    writer = csv.writer(f)
