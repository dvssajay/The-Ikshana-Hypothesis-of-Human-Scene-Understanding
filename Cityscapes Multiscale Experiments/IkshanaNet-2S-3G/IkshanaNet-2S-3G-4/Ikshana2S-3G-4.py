...
train , dump = torch.utils.data.random_split(train_ds, [185,2790], generator=torch.Generator().manual_seed(42))
...
class IkshanaNet(nn.Module):
  def __init__(self):
    ...
    self.s1 = nn.Sequential(nn.Conv2d(96, 20, kernel_size=1, stride =1,bias=False),
                                   nn.BatchNorm2d(20),
                                   nn.ReLU(inplace=True))
    ...
    self.s2 = nn.Sequential(nn.Conv2d(192, 20, kernel_size=1, stride =1,bias=False),
                                   nn.BatchNorm2d(20),
                                  nn.ReLU(inplace=True))

 
    self.out = nn.Sequential(nn.Conv2d(40, 20, kernel_size=1, stride =1,bias=False))
  ...
  def forward(self,x):
    ...
    s2 =  torch.nn.functional.interpolate(s2, (512,1024), mode='bilinear',align_corners=True)
...
def train_val(model, params):
    ...
    best_loss=float('inf')
    o = open('./IkshanaNet-2S-3G-4/IkshanaNet-2S-3G-4-Trainingset.txt','w')
...
import os
path2models= "./IkshanaNet-2S-3G-4/IkshanaNet-2S-3G-4-Trainingset_"
...
o = open('./IkshanaNet-2S-3G-4/IkshanaNet-2S-3G-4_time.txt','w')
...
plt.legend()
plt.savefig('./IkshanaNet-2S-3G-4/IkshanaNet-2S-3G-4.png', dpi = 300)
...
with open('./IkshanaNet-2S-3G-4/plot.csv', 'w') as f:
    writer = csv.writer(f)
