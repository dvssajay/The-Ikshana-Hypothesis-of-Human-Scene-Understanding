...
class IkshanaNet(nn.Module):
  def __init__(self):
    ...
    self.s2 = nn.Sequential(nn.Conv2d(128, 20, kernel_size=1, stride =1,bias=False),
                                   nn.BatchNorm2d(20),
                                  nn.ReLU(inplace=True))
    ...
    self.s3 = nn.Sequential(nn.Conv2d(192, 20, kernel_size=1, stride =1,bias=False),
                                  nn.BatchNorm2d(20),
                                  nn.ReLU(inplace=True))
...
def train_val(model, params):
    ...
    best_loss=float('inf')
    o = open('./IkshanaNet-3S-2G-4/IkshanaNet-3S-2G-4-Trainingset.txt','w')
...
import os
path2models= "./IkshanaNet-3S-2G-4/IkshanaNet-3S-2G-4-Trainingset_"
...
o = open('./IkshanaNet-3S-2G-4/IkshanaNet-3S-2G-4_time.txt','w')
...
plt.legend()
plt.savefig('./IkshanaNet-3S-2G-4/IkshanaNet-3S-2G-4.png', dpi = 300)
...
with open('./IkshanaNet-3S-2G-4/plot.csv', 'w') as f:
    writer = csv.writer(f)
